...
information. Sub-routines and object fields also have type information which we
need to keep track of, but the information is not the same. For instance, a
...
and possible properties. Similarly an object field will have (TODO). %XXX
...
different places, there is a centralized table, containing all meta
information. This is named the \term{metadata table} and will need to be
dynamically sized to support reflection and generating new types at run-time.
%TODO lidt forvirrende
...
will be a simple integer array, mapping binary metadata indices to the machine's
internal metadata table entries. The metadata entries will be parsed lazily,
i.e.~the first time the program tries to look up a metadata entry through its
index in the binary meta table, it is parsed and stored in the machine's meta
table. %TODO lidt forvirrende
...
look it up through a look-up function, which takes a metadata hash. This can be
done through a strict function which thrown an exception if the hash is not
found, or a lazy function which will generate the new meta entry if not
found. This is especially useful for looking up types that are not guaranteed to
already be found in the meta table, such as reference types used when boxing
(see Section~\ref{NEEDED}).
...
two infinities (plus and minus) and a NaN (not a number).
...
case the machine itself needs to make a choice about is with stack elements of
type {\tt AnyType}. Instead of having a long list of what do do in every case of
all types, the choice will be defined by a type lattice. This type lattice will
control what type the result is of, in a given operation between two numbers of
different types.
...
are not implemented. The version element is to support dynamic languages (TODO)
