...
sub-routine on heap objects called \code{finalize} which is called before the
...
structures for organizing bits and bytes into sections and defining headers that
...
the file can be executed, endianess, ELF format version and so on. Further it
...
displacement, without removing it, and push it to the top of the stack. The
...
has pushed an element to the stack with the sole purpose of being
overwritten. %TODO wat? watfix?

There will be separate instructions for adding new elements onto the
stack, namely \instr{pushLiteral} and \instr{pushConstant}, making the
\instr{push} instruction obsolete.
...
%XXX jeg synes at hele det her afsnit mangler noget arbejde. watfix?
...
%TODO wat? @første sætning watfix?
...
concurrently. A process utilizing this feature is called multi-threaded. %XXX watfix?
...
% TODO: more on prefixes. what about suffixes? XXX
...
\item[\instr{pushMeta}:]
  ...
  encompassing type-, field- and sub-routine information. The \instr{pushMeta}
...
(numbers are in binary representation): % TODO: mention endianness earlier
\begin{align*}
