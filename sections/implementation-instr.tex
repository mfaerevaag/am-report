...
Standard Library has built-in constants for finding a float's maximum value;
...
built-in arithmetic operations on them. This is unfortunate, as {\tt FLT\_MAX -
  1}, for instance, is regarded as undefined behavior and will on most platforms
be equal to {\tt FLT\_MAX}. Inducing undefined behavior when trying to deduce
whether an instruction is safe, renders it's point moot. %TODO lidt forvirrende
...
will be thrown, even though under- or overflow might not have occurred. TODO:
need more comment?
...
%XXX står også i design (måske flere steder)
The machine also supports saturated version of the arithmetic instructions. The
saturated variants ensure that overflow will result in the types respective
maximum or minimum value, instead of creating undefined behavior. As an example
