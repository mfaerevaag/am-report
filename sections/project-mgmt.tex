...
process, and that it pays off to thoroughly think about design choices before
starting an implementation. It is a fine balance between management and
production and there will never be a single method that works for all types of
projects. We consider the essence of the agile methodology great, but the
...
to the end-user is, how it is going to be deployed (continuous delivery, large
milestones or a single monolithic delivery), the size of the team and so on.
