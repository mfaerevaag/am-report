...
which could be proved by induction. The benchmark result is shown below, in
...
optimizations such as tail-recursion, which would have an significant impact on
the running time.
...
$T(n) = O(n) * k * O(1) = O(n)$, where $k$ is the number of stack
...
the same as the sub-routine benchmark; $T(n) = O(n) * k * O(1) = O(n)$, where
$k$ is the number of stack operations. The result of the
benchmark can be seen below, in figure~\ref{fig:eval:benchmark:heap}.
\begin{figure}[H]
  \centering
  \scalebox{0.8}[0.7]{\input{figures/heap}}
...
\begin{figure}[H]
  \centering
  \scalebox{0.8}[0.7]{\input{figures/jit}}
  \caption{JIT comparing on JVM}
\label{fig:eval:benchmark:jit}
\end{figure}

We can see the huge benefit JIT compilation TODO
